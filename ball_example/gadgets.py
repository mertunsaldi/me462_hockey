...
class PlotClock(Gadgets):
    ...
    def __init__(
        ...
        *,
        ...
        def _safe_query(expr: str) -> Optional[float]:
            try:
                return float(self._query_value(expr))
            except Exception:
                return None
        ...
        self.max_x = max_x if max_x is not None else 80.0
        ...
        self.y_range = (
            self.min_y,
            math.sqrt(
                max(0.0, self.workspace_radius * self.workspace_radius - self.workspace_dist * self.workspace_dist)
            ),
        )
    ...
    def _query_value(self, code: str, timeout: float = 1.0) -> str:
        ...
        if not code.startswith(prefix):
            ...
            cmd = code
        # clear any pending lines so the next response we read corresponds to
        # this command. this prevents a late arrival from a previous query from
        # being mistaken for the new response.
        ...
        while time.time() - start < timeout:
            ...
            for line in lines:
                if line.startswith(f"P{self.device_id}:"):
                    ...
                    # allow any additional output generated by this command to
                    # arrive before the next query
                    time.sleep(0.05)
                    self.master.get_lines()
